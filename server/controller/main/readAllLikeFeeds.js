...
module.exports = async (req, res) => {
  ...
  try {
    ...
    logger(
      `다운로드한 피드 조회 - 유저 ${userId}: 모든 다운로드한 피드 조회. 옵션(start: ${start}, end: ${end}, limit: ${limit}, order: ${order})`
    );
    ...
    logger(
      `[ERROR] 다운로드한 피드 조회 - 유저 ${userId}: 서버 에러. 모든 다운로드한 피드 조회. 옵션(start: ${start}, end: ${end}, limit: ${limit}, order: ${order})`
    );
    console.error(error);
