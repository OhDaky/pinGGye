...
module.exports = async (req, res) => {
  ...
  try {
    ...
    logger(`피드 조회 - 유저 ${userId}: 모든 피드 조회. 옵션(start: ${start}, end: ${end}, limit: ${limit}, order: ${order})`);

    res
      .status(200)
      .json({ data: { feeds }, message: "All feeds successfully read" });
  } catch (error) {
    logger(`[ERROR] 피드 조회 - 유저 ${userId}: 서버 에러. 모든 피드 조회 요청 실패. 옵션(start: ${start}, end: ${end}, limit: ${limit}, order: ${order})`);
    console.error(error);
