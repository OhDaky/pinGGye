...
module.exports = async (req, res) => {
  ...
  if (!feedId || !textContent) {
    logger(
      `[ERROR] 피드 댓글 입력 - 유저 ${userId}: 요청 파라미터 부족. feedId: ${feedId} textContent: ${textContent}`
    );
    return res
      .status(400)
  ...
  try {
    ...
    logger(
      `[ERROR] 피드 댓글 입력 - 유저 ${userId}: 유효하지 않은 피드 ${feedId}번`
    );
    ...
    console.error(error);
    logger(
      `[ERROR] 피드 댓글 입력 - 유저 ${userId}: 서버 에러. 피드 ${feedId}번 댓글 입력 요청 실패`
    );
